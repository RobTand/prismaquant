...
# ---------------------------------------------------------------------------
# imatrix values from sensitivity-probe marginals, and their canonical digest.
#
# The activation snapshot cache is row-capped because its job is replay, not
# population statistics. The sensitivity probe already accumulates the full
# calibration-corpus second moment for every input column, so an imatrix can
# be read off the existing ``act_sq_sum`` / ``n_tokens_seen`` fields (or their
# packed-expert counterparts) without creating a cache. The col-weights harvest
# in ``run-pipeline.sh`` consumes these for the GGUF lane and every
# imatrix-weighted render. Moved here from ``cb_imatrix`` on 2026-09-25
# (#1304); the persisted schema strings keep their historical values.
# ---------------------------------------------------------------------------
IMATRIX_FROM_PROBE_SCHEMA = (
    "prismaquant.cb_imatrix.probe_act_sq_sum_over_tokens.v1"
)
IMATRIX_VALUE_HASH_SCHEMA = "prismaquant.cb_imatrix_values.v1"
...
def canonical_imatrix_sha256(
    ...
    if not isinstance(col_weights, Mapping) or not col_weights:
        raise ValueError("imatrix values must be a nonempty mapping")
    digest = hashlib.sha256()
    digest.update((IMATRIX_VALUE_HASH_SCHEMA + "\0").encode("utf-8"))
    if any(not isinstance(name, str) or not name for name in col_weights):
        raise ValueError("imatrix qnames must be nonempty strings")
    for name in sorted(col_weights):
        ...
        if value.numel() == 0 or not bool(torch.isfinite(value).all()):
            raise ValueError(f"{name}: imatrix must be finite and nonempty")
        if bool((value < 0).any()):
            raise ValueError(f"{name}: imatrix cannot contain negative values")
        shape = json.dumps(
