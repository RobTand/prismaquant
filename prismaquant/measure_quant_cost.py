...
def _accumulate_result(bucket: dict, name: str, fmt: str,
    ...
    if cost_source is not None:
        ...
        # but not why the row has none. A rung that the DP later selects
        # must be able to say where its price came from.
...
def _load_unrouted_expert_declaration() -> frozenset[str]:
    ...
    a neutral prior. Unset -> empty set -> the coverage gate behaves exactly
    ...
    if report.get("rule") != "unrouted_expert_neutral_prior:layer_routed_mean":
        raise ValueError(
            ...
            f"{report.get('rule')!r}; refusing to widen the coverage gate "
...
def _cost_render_uses_imatrix(spec: fr.FormatSpec) -> bool:
    ...
    export is optionally --imatrix-from-act-cache); no other family is."""
    return spec.family == "gguf" and _gguf_imatrix_enabled()
...
def _batched_quantize(
    ...
    if spec.name in _EXPORT_ALIGNED_BATCH_FORMATS:
        ...
        # MXFP8_UE8M0_G32 is "fp8_e4m3" like MXFP8_E4M3, but its shared
...
def _cb_col_weights_lookup(name: str):
    """Shared col-weights pickle (PRISMAQUANT_CB_COL_WEIGHTS; the name predates
    the retired codebook lane's archive, 2026-09-25, #1304), set when local
    packed-expert costs must match the exporter's weighting — incl. the
    synthesized per-expert down_proj replay entries the inline module-input
    pool can never provide."""
    global _CB_CW_CACHE
...
def measure_batched_gpu(model: nn.Module, act_cache: "ActivationIndex",
    ...
    for (in_f, out_f), entries in groups.items():
        ...
        # They still need a priced row or the allocator's coverage gate
        ...
        for chunk_i, chunk in enumerate(chunks_list):
            ...
            def _measure_spec_into_accum(spec, idx=None):
                ...
                or an index-subset. Identical math either way."""
