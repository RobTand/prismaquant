...
deployed generation pinned by tests/produced_render_pb_pin.json carries
it: PoolQueue.validate_produced_output_batch plus the full prepaid
lifecycle including the DEV null-digest descriptor path).

Rebased onto the deployed generation for PQ #870 from PR #871 (which
integrated the same lifecycle against the then-unqualified candidate);
the lifecycle shape -- bound-admitted budget, proven reuse, DEV null
digest, filed CAS root -- is preserved from that reviewed change.
"""
