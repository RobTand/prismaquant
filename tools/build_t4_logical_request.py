...
def main():
 p=argparse.ArgumentParser(description=__doc__);p.add_argument('--root',required=True,help='T4 campaign root: pilot24-results.json and qualified/');p.add_argument('--catalog',required=True);p.add_argument('--catalog-sha256',required=True);p.add_argument('--python',required=True,help='interpreter on the worker, pinned to the current Tessera');p.add_argument('--qualifier-checkout',required=True);p.add_argument('--out',required=True);args=p.parse_args();ROOT=Path(args.root)
 raw=Path(args.catalog).read_bytes();assert sha(raw)==args.catalog_sha256;catalog=json.loads(raw)
 ...
 request={'schema':'prismabuild.logical_request.v1','common':{'argv':[args.python,'tools/qualify_t4_overlay.py','--pb-task-batch','{pb.task_batch}','--allowed-tiers','ram,ssd'],'cwd':args.qualifier_checkout,'demand':{'cpu':3,'mem_gb':4,'gpu':1},'gpu_memory_gb':2,'data_manifest':None,'env':{'OMP_NUM_THREADS':'1','MKL_NUM_THREADS':'1','OPENBLAS_NUM_THREADS':'1','PYTHONPATH':'.','T4_QUALIFY_READ_WORKERS':'2'},'tags':['gb10'],'timeout_s':600},'roster':{'schema':'prismabuild.logical_task_roster.v1','tasks':tasks},'batch_policy':{'schema':'prismabuild.roster_batch_policy.v1','residencies':[{'key':'retained-glm-a4','setup_seconds':9,'setup_evidence':'PB pilot24 wall15s minus ~10.5s percell work plus cold firstcell4.4s; conservative startup allowance, not throughput measurement'}],'max_setup_fraction':0.1,'max_estimated_wall_seconds':120},'task_data_manifest':{'schema':'prismabuild.task_data_manifest.v1','payload_field':'reads','mount_prefix':'/mnt/shared','residency_tier':None,'residency_ram':'off','mover_readers':2,'mover_mem_gb':1}}
 out=Path(args.out);raw=(json.dumps(request,sort_keys=True,separators=(',',':'))+'\n').encode()
 with out.open('xb') as f:f.write(raw)
 print(json.dumps({'path':str(out),'sha256':sha(raw),'tasks':len(tasks),'reused':len(existing),'status':'ready_for_verified_PB_client62047'}))
