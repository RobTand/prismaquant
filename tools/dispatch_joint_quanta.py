...
class ExecutableBindingUnsupported(DispatchRefused):
    ...
    An executable manifest is admitted for production dispatch only when
    its sealed ``render_prerequisite.binding`` is a produced-output batch
    reference the real queue validator accepts
    (``PoolQueue.validate_produced_output_batch`` through the injected
    binding validator). A sequencing-only manifest (``binding: None``), a
    missing validator, or a refused reference keeps this typed refusal --
    however plausible the prerequisite dictionary looks. Sequencing/phase
    artifacts remain useful for read-plan qualification; production
    dispatch of the legacy slice rows is unchanged.
    """
...
def _executable_row_parts(record: dict, *, output_root: Path,
    ...
    gate: production dispatch (:func:`quantum_argv`) admits an executable
    row only on a binding the real queue validator accepts and refuses
    every other executable row with :class:`ExecutableBindingUnsupported`
    before reaching here. Tests exercise manifest/phase propagation
    through this helper directly.
    """
