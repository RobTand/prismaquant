...
`nvfp4_fused.py` and three producer-side kernels:
`joint_projection_reduce` and the Stage B KDA capture kernel `kda_chunk.py`
(`kda_gram_v1`, PQ #1199), each with a packaged qualification file, and the
served NVFP4 activation leg's dequantisation `nvfp4_served_dequant.py` (PQ
#1211, bit-identical to the Torch leg it replaced). None of them is a serving
