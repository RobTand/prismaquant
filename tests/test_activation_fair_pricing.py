...
``PRISMAQUANT_EXPERT_COST_SAMPLE`` are written with
``output_mse_measured=False``, so they fall to weight-only Fisher pricing where
the activation contract is structurally invisible: a W4A4 rung gets credit for
its cheaper weight stream with none of its A-side cost.

What this file pins:

1. The fit is a per-family GEOMETRIC MEAN of the measured-over-weight-only
   estimator ratio, exact on synthetic rows, for both families.
...
# NOTE 2026-09-25 (#1304): these cases priced two codebook families until the
# retired codebook lane was archived. They now price the live W4A4-vs-W8A8
# contest the pricing exists for: NVFP4 (family ``nv``) against FP8_E4M3
# (family ``fp``). The within-family rung-dependence case uses the two ``mx``
# rungs. The arithmetic under test is unchanged.
...
# One activation-quantizing rung per family: NVFP4 is W4A4 (family ``nv``),
# FP8_E4M3 is W8A8 (family ``fp``, which BF16 shares as an identity rung).
_FP4_RUNGS = ["NVFP4"]
_FP8_RUNGS = ["FP8_E4M3"]
...
def _shape_stats(h_trace: float) -> dict:
    # in_features % 32 == 0 and out_features % 16 == 0 keeps every rung
    # legal, so nothing drops out for shape reasons here.
    return {
        "h_trace": h_trace,
        "n_params": 1024 * 512,
        "in_features": 1024,
...
def test_family_penalty_is_the_geometric_mean_of_the_estimator_ratio():
    """Exact on synthetic rows, per family, for both families.
    ...
    """
    rows = [
        CalibrationRow("a", "NVFP4", "nv", 4.0, 1.0),
        CalibrationRow("b", "NVFP4", "nv", 16.0, 1.0),
        CalibrationRow("c", "FP8_E4M3", "fp", 2.0, 1.0),
        CalibrationRow("d", "FP8_E4M3", "fp", 8.0, 1.0),
    ]
    fit = calibrate(
        rows,
        measured_rows_by_family={"nv": 2, "fp": 2},
        weight_only_rows_by_family={"nv": 7, "fp": 7},
    )
    assert fit.enabled
    assert fit.families["nv"].penalty == pytest.approx(8.0)
    assert fit.families["fp"].penalty == pytest.approx(4.0)
    ...
    assert fit.penalty_for("NVFP4", True) == (
        pytest.approx(8.0), BRANCH_CALIBRATED)
    assert fit.penalty_for("FP8_E4M3", True) == (
...
def test_provenance_records_the_sample_the_fit_and_the_residuals():
    ...
    rows = [
        CalibrationRow(f"layer.{i}", "NVFP4", "nv",
                       2.0 ** (i + 1), 1.0)
        for i in range(4)
    ]
    fit = calibrate(
        rows,
        measured_rows_by_family={"nv": 4},
        weight_only_rows_by_family={"nv": 3},
    )
    payload = fit.as_dict()
    family = payload["families"]["nv"]
    ...
    assert (shuffled.families["nv"].rows_digest
            == fit.families["nv"].rows_digest)
...
def test_rung_dependence_of_the_ratio_is_recorded_not_hidden():
    ...
    rows = [
        CalibrationRow("a", "MXFP4", "mx", 2.0, 1.0),
        CalibrationRow("b", "MXFP4", "mx", 2.0, 1.0),
        CalibrationRow("c", "MXFP8_E4M3", "mx", 8.0, 1.0),
        CalibrationRow("d", "MXFP8_E4M3", "mx", 8.0, 1.0),
    ]
    fit = calibrate(
        rows,
        measured_rows_by_family={"mx": 4},
        weight_only_rows_by_family={"mx": 1},
    )
    family = fit.families["mx"]
    ...
    assert per_format["MXFP4"] == pytest.approx(1.0)
    assert per_format["MXFP8_E4M3"] == pytest.approx(3.0)
...
def _two_family_tables(*, fp4_weight_only=True, fp8_weight_only=True):
    ...
    for i in range(2):
        ...
        costs[name] = {
            ...
            "NVFP4": _measured_row(weight_mse=1e-4, output_mse=4e-4),
            # ...and 2x for the fp8 family.
            "FP8_E4M3": _measured_row(weight_mse=1e-4, output_mse=2e-4),
...
def test_calibration_corrects_weight_only_rows_and_leaves_measured_rows_alone():
    ...
    assert priced["NVFP4"].predicted_dloss == pytest.approx(4e-3)
    assert priced["FP8_E4M3"].predicted_dloss == pytest.approx(2e-3)
    assert priced["NVFP4"].activation_pricing == BRANCH_CALIBRATED
    ...
    assert dense["NVFP4"].predicted_dloss == pytest.approx(0.5 * 4e-4)
    assert dense["NVFP4"].activation_pricing == BRANCH_MEASURED
...
def test_the_correction_is_exactly_the_difference_from_pre_p5a_pricing():
    ...
    stats, costs, expert = _two_family_tables()
    pricing = calibrate_activation_fair_pricing(stats, costs, _specs())
    entry = costs[expert]["NVFP4"]
    before = cost_entry_predicted_dloss(
        stats[expert], entry, format_name="NVFP4")
    after = cost_entry_predicted_dloss(
        stats[expert], entry, format_name="NVFP4",
        activation_pricing=pricing)
    assert after == pytest.approx(
        before * pricing.families["nv"].penalty)
...
def test_calibration_rows_are_collected_only_from_activation_quantizing_formats():
    ...
    rows, measured, weight_only = collect_activation_calibration_rows(
        stats, costs, _specs())
    assert {row.family for row in rows} == {"nv", "fp"}
    ...
    # although it shares family ``fp`` with FP8_E4M3. Two dense rows of
    # FP8_E4M3 are measured; the two BF16 rows beside them are not counted.
    assert all(row.fmt != "BF16" for row in rows)
    assert measured == {"nv": 2, "fp": 2}
    assert weight_only == {"nv": 1, "fp": 1}
...
def test_a_mixed_cost_scale_refuses_and_names_the_env_var():
    ...
    WORSE than the status quo: it tilts exactly the NVFP4-vs-FP8
    ...
    assert "'fp'" in message and "'nv'" in message
...
def test_no_measured_activation_rows_passes_through_but_is_recorded():
    ...
    assert pricing.weight_only_rows_by_family == {"nv": 3, "fp": 3}
    assert set(pricing.uncalibrated_families) == {"nv", "fp"}
...
def test_a_fused_super_item_folds_the_family_penalty_in_exactly_once():
    ...
    for i in range(2):
        ...
        costs[dense] = {
            "NVFP4": _measured_row(weight_mse=1e-4, output_mse=3e-4),
            "BF16": {"weight_mse": 0.0, "output_mse": 0.0,
                     "output_mse_measured": True},
        }
    specs = _specs(["NVFP4", "BF16"])
    pricing = calibrate_activation_fair_pricing(stats, costs, specs)
    assert pricing.families["nv"].penalty == pytest.approx(3.0)
    ...
    super_cand = next(
        c for c in cands_ext[super_name] if c.fmt == "NVFP4")
...
def test_the_dp_flips_to_fp8_once_the_w4_activation_cost_is_priced():
    """The audit's whole point, end to end through the real knapsack.

    An expert unit's weight-only rows make NVFP4 look strictly better than
    FP8 per byte, so the DP takes the W4 rung: it gets credit for its
    cheaper weight stream with none of its A-side cost. The measured
    ...
    for i in range(2):
        ...
        stats[name] = _shape_stats(1.0)
        costs[name] = {
            "NVFP4": _measured_row(weight_mse=1e-5, output_mse=3e-5),
            "FP8_E4M3": _measured_row(weight_mse=1e-5, output_mse=1e-5),
            "BF16": {"weight_mse": 0.0, "output_mse": 0.0,
                     "output_mse_measured": True},
        }
    # The expert unit: weight-only rows only (PRISMAQUANT_EXPERT_COST_SAMPLE).
    # NVFP4 (4.5 bpw) is both CHEAPER in bytes and priced lower than FP8_E4M3
    # (8 bpw plus one tensor scale): dominant, on weight-only evidence alone.
    expert = "model.layers.9.mlp.experts.gate_up_proj"
    stats[expert] = _shape_stats(1.0)
    costs[expert] = {
        "NVFP4": _weight_only_row(1.0e-3),
        "FP8_E4M3": _weight_only_row(1.2e-3),
        "BF16": _weight_only_row(0.0),
    }
    ...
    def _solve(pricing):
        cands = build_candidates(
            stats, costs, specs, target_profile="research",
            activation_pricing=pricing)
        # Budget the expert row alone at a rate both quantized rungs can
        # reach and BF16 cannot.
        solved = solve_allocation(
            {expert: stats[expert]}, {expert: cands[expert]}, 8.5)
        assert solved is not None
        assignment, _chosen = solved
        return assignment[expert]

    assert _solve(None) == "NVFP4"                 # pre-P5a: W4 wins
    pricing = calibrate_activation_fair_pricing(stats, costs, specs)
    assert pricing.families["nv"].penalty == pytest.approx(3.0)
    assert pricing.families["fp"].penalty == pytest.approx(1.0)
    assert _solve(pricing) == "FP8_E4M3"           # activation-fair: W8 wins
...
def test_branch_label_is_derivable_without_a_calibration_object():
    ...
    assert cost_entry_activation_pricing_branch(
        stats, _weight_only_row(1e-3), "NVFP4", None
    ) == BRANCH_UNCALIBRATED
    assert cost_entry_activation_pricing_branch(
