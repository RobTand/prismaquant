...
def test_a_staged_range_offers_the_ram_copy_and_retires_it_with_the_epoch(
        tmp_path, monkeypatch):
    cell, wire, blob, staged, ram, map_path = _ram_wire(tmp_path)
    monkeypatch.setenv(ENV_VAR, str(map_path))
    _bind()
    resolver = residency_resolver()
    start, end = 8, 16
    offered = resolver.staged_range(wire, start, end, declared_size=len(blob))
    assert offered['ram_path'] == str(ram['w'])
    # The tmpfs died between lookups: the record now announces another epoch,
    # written in place, and the next lookup reads the new one.
    _announce(tmp_path, epoch=LATER_EPOCH)
    offered = resolver.staged_range(wire, start, end, declared_size=len(blob))
    assert 'ram_path' not in offered and offered['stage_path'] == str(staged['w'])
