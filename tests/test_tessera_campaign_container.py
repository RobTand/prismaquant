...
def test_the_launcher_supplies_the_bounded_capture_environment(tmp_path):
    ...
    # The cap is still the spec's own field, and it is still the CPU cap.
    assert f"--memory" in argv
    assert argv[argv.index("--memory") + 1] == "21g"
    assert argv[argv.index("--memory-swap") + 1] == "21g"
