...
RESEARCH_FAMILIES = E4M3 + ['TESSERA_BF16_K1_R1024', 'TESSERA_E2M1_K2_R896']
OTHER = ['NVFP4', 'FP8_E4M3']
...
@pytest.mark.parametrize('fmt', RESEARCH_FAMILIES + ['BF16'])
...
@pytest.mark.parametrize('fmt', RESEARCH_FAMILIES + OTHER + ['UNKNOWN'])
...
def test_actual_candidate_filter_masks_before_routed_grouping(fmt, qname, packed):
    ...
    expected = {'BF16', fmt} if fmt in RESEARCH_FAMILIES else {'BF16'}
    assert {candidate.fmt for candidate in result[qname]} == expected
    if fmt not in RESEARCH_FAMILIES:
...
def test_routed_allow_list_retains_the_full_research_menu_without_serving_claim():
    """This emulation-only allocation menu must not shrink to current cells.

    The full menu preserves an allocator's selection domain. A later serving
    route decides whether a selected family can execute; profile admission is
    deliberately not a claim that a routed cell is qualified.
    """
    profile = sp.load_serving_profile(PROFILE)
    rule, = [r for r in profile.format_rules
             if r.id == 'glm_routed_packed_research_families']
    assert set(rule.allow_tessera_families) == {
        'TESSERA_E4M3_K1', 'TESSERA_BF16_K1', 'TESSERA_E2M1_K2'}
    assert profile.emulation_only and profile.export_lane is None
