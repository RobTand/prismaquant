...
| `PRISMAQUANT_STAGE_A_DECLARED_CHECKPOINTS` | unset (off) | `joint_cost_stage_a.py` (`resolve_declared_checkpoints`, `run_adjoint_capture`), `joint_adjoint_checkpoints.py` (`_write_declared_adjoint_checkpoint`), `cost_streaming.py` (`reserve_checkpoint_artifact`, `commit_checkpoint_artifact`, `_retire`) | Exactly `1` writes each Stage A adjoint checkpoint as a declared checkpoint: the record (schema `prismaquant.joint_adjoint_checkpoint.v2`) references the owner's existing cotangent entries instead of copying them, commit pins those entries so the rolling chain never unlinks them, and with a bound local output spool the checkpoint's own files go through it as one `checkpoint`-class PrismaBuild group rather than synchronously into the shared output root. The template's `output_prefix` must cover `<space>/checkpoints`, or the write refuses before any byte. Unset or `0` is byte-identical to the copy writer; any other value refuses. It is carried in the sealed producer environment. ARCHITECTURE.md, "declared adjoint checkpoints". Tests: `tests/test_declared_adjoint_checkpoints.py`, `tests/test_declared_adjoint_checkpoints_real_pb.py`, `tests/test_adjoint_checkpoint_default_path_identity.py`. |
