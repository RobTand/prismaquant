...
| `PRISMAQUANT_AQUA_ACT_WEIGHT_BASIS` | unset (= `source`) — **RESEARCH lever** | `aqua_activation_cost.py` (`resolve_act_weight_basis`, `rendered_weight_for_basis`, read by `activation_dloss_table`); the seam is `format_cost_protocol.price_activation_only(rendered_weight=...)` and the render is `format_cost_registry.RegistryFormatPlugin.render` | Render-conditioned A-side: evaluate the activation term over the format's rendered `W_hat_f` instead of the source `W`, i.e. `E_a = (1/2T)·Σ_o g_sq[o]·Σ_j W_hat_f[o,j]²·ν_j`. The local error decomposes as `W_hat x_hat − W x = dW·x + W_hat·dx`, so the activation term belongs on `W_hat_f`; putting it there absorbs the `dW·dx` contribution the source-weight form drops. **Still dropped on every basis:** the `dW·x` / `W_hat·dx` cross-correlation, downstream nonlinear interactions, and MoE routing interactions (a route flip is invisible to any second-moment term). Accepted values are `source` (the default and a byte-identical no-op), `rtn` (the registry's own `quantize_dequantize`) and `compensated` (the GPTQ+JSO tensors from `--production-cache`, i.e. the bytes the export ships). A malformed value is a hard `SystemExit`, never a silent revert. Neither rendered basis is ever a fallback for the other: `compensated` without a cache refuses, a cache supplied on another basis refuses, a (unit, format) the cache lacks is a counted HOLE (`render_misses`), and zero render coverage refuses rather than writing a no-op. `ν` is unaffected — it is a property of the input distribution, measured by replaying the format's own activation QDQ on the probe's cached rows when `--act-dir` is given, and otherwise an explicitly stamped Gaussian **approximation** that cannot represent a clipping tail. **Status: research.** On a 24-unit fixture the **`rtn`** basis moved `E_a` by a median 0.05% (NVFP4 span 0.955–1.019×), Spearman 1.000 with **0 rank moves**, and produced **identical allocations** at every budget and A/W ratio tested. **That null is scoped to `rtn`: the `compensated` delta is unmeasured** — GPTQ's `dW` is deliberately biased, so the production render is where `W_hat²` could depart from `W²` most, and the two bases must not be assumed interchangeable. Correctness of `compensated` (dense and packed) is tested against a planted cache; its size against the source basis is not. No served A/B on either basis. Tests: `tests/test_aqua_render_conditioned_act_cost.py`. |
