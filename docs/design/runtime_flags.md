...
on that pass.*
*2026-09-20: §1 gains `PRISMAQUANT_DEV_PROGRESS_STAMP` and the
`PRISMAQUANT_DEV_MODE` row is corrected to it — per-progress-record dev stamps
are OPT-IN (PR #826/#828), so a default dev run's durable-unit commits carry
the certified six-field record and pay no sealing ceremony. Certified mode and
the run-level `dev_uncertified` identity in `results.json` are unchanged. No
other row re-verified on that pass.*
*2026-09-20: the `PRISMAQUANT_DEV_MODE` row (§1) gains the dev-mode
merged-checkpoint seal skip (#833) — policy on an existing variable, not a new
one. No other row re-verified on that pass.*
*2026-09-23: §4 gains `PRISMAQUANT_BF16_REDUCED_PRECISION_REDUCTION`
(PQ #1028), the launch setting for the bf16 reduced-precision reduction flag.
No other row re-verified on that pass.*
*2026-09-23: §4 gains `PRISMAQUANT_STAGE_A_KERNEL_PROFILE` (#899) and
`PRISMAQUANT_STAGE_B_KERNEL_PROFILE` (PQ #1029), the opt-ins for the kernel-time
profiler sessions. No other row re-verified on that pass.*
...
|---|---|---|---|
| `PRISMAQUANT_DEV_MODE` | unset (certified) | the gates: `prismaquant/dev_mode.py` (`dev_mode_enabled`); `joint_aura_run_transition.source_proof` + the loader identity family; `tessera_joint_aura._preflight_run_prepared` and the run-path prepared loop (`_prepared_digest_recorded`), `execute`'s results.json stamp, `_pb_commit`, `load_measured_anchor_input`'s campaign checkpoint seal; `aura_cost._prepare_aura_checkpoints`; `dispatch_tessera_campaign._bound_sha256` and `_submit_gpu_action`'s spec-env merge | **The owner's rapid-iteration switch (Rob, 2026-09-19, #771): the seal returns at the artifact gate, not the run gate.** `1` — exactly the string `1` — turns the run-gate provenance walls into stamps: any executing package is admitted under a source-transition receipt (the record carries the package's ACTUAL tree digest — a record, never a gate), the prepared record's `plan_sha256`/`implementation_sha256` comparisons warn with both digests and continue (every other prepared field still walls), a mismatched AURA checkpoint lineage is archived whole as `<dir>.dev-archived-<iso>` and recomputed fresh (never silently reused; corrupt checkpoints still refuse), and the submitter treats declared digests as records while sealing the variable into the container spec env exactly as `PRISMAQUANT_LAYER_READ_THREADS` travels. The merged campaign checkpoint's canonical seal is not recomputed in dev mode: the loader uses the manifest's declared `identity_sha256` after a 64-hex shape check, records the skip in a `[DEV-MODE]` line, and leaves the raw checkpoint file's `_bound` SHA-256 check above the gate in force; the declared seal itself is recorded, not verified, and the 302.653 s encode measured on action `282c61140ba7` (2026-09-20) is paid only in certified mode. Every dev run writes `{"dev_uncertified": true, "dev_mode": {…}}` top-level in `results.json`, and every suspended gate prints a `[DEV-MODE]` line; progress records carry the stamp only under the opt-in row below, so the default per-unit commit stays the certified six-field shape. Any other value — unset, `0`, `true` — is certified mode, byte-identical to the pre-switch behavior. PrismaBuild's sealed actions and the v1/v2 proofs are untouched. Tests: `tests/test_dev_mode_provenance_gates.py`, `tests/test_dev_anchor_seal_skip.py`. |
| `PRISMAQUANT_DEV_PROGRESS_STAMP` | unset (off) | `tessera_joint_aura.py:494` (`_pb_commit`) | Opt-in per-line dev stamp. Only when this is exactly `1` **and** `PRISMAQUANT_DEV_MODE=1` does each progress record carry the `dev_uncertified`/`dev_mode` block (with the executing tree's digest, memoized once per process by #827); unset, the record is byte-identical to the certified six-field shape (`schema`, `token`, `phase`, `units_completed`, `unit`, `reported_unix`) even in dev mode, and no source hash runs. Rob's campaign directive (2026-09-13) is that dev runs pay no sealing ceremony, and the run identity already lives once in `results.json` (row above); before #827 the per-line form re-hashed the whole tree on every durable unit (#826). Certified mode is untouched either way. Tests: `tests/test_dev_mode_provenance_gates.py`, `tests/test_pb_commit_source_memo_826.py`. |
...
| `PRISMAQUANT_SOURCE_HASH_THREADS` | auto (`len(os.sched_getaffinity(0))`, never more than the number of uncached shards) | `cost_streaming.py` (`source_identity_hash_threads`, `_hash_source_shards`) | Worker count for hashing the source shards that `build_source_checkpoint_identity` has no digest-cache entry for. Each worker reads one shard in 16 MiB blocks and runs sha256 on it. `hashlib` releases the GIL on those updates, so the pool uses the cores the process is admitted to. Each shard's stat fingerprint is taken before hashing and checked again afterward, and a changed shard is still refused. When several shards fail, the error is the first one in sorted shard order. The identity dict and the digest-cache file are byte-identical to the serial loop. Measured cold (pages evicted) on the sparky GB10 NVMe, 16 x 1 GiB shards, `--cpus 20`: serial 12.51 s, 16 threads 2.16-2.29 s (5.5x), 4 threads 4.05 s. The default has no fixed I/O cap. On slow storage, such as an NFS export backed by a spinning-disk pool, measure first and set this lower, because that path has not been measured. `1` restores the serial loop. Tests: `tests/test_source_checkpoint_identity_parallel_hash.py`. |
| `PRISMAQUANT_STAGEA_STAGER` | unset: a bound Stage A owner whose sealed window is wider than two groups runs its PrismaBuild calls on one background stager thread; at the default two-group window there is no thread either way | `cost_streaming.py` (`StreamedBoundaryArtifacts._produced_start_stager`), `produced_stager.py` | `inline` keeps every step on the calling thread at any window width, which is the synchronous read-ahead loop of PQ #887. It is a run-scoped override for a run that must rule the thread out, and the before arm of a comparison: the owner counts `produced_compute_blocked_s` in both modes. Any other value is the default. It changes no sealed plan, no kernel order and no bytes written. ARCHITECTURE.md, "The stager thread (PQ #895)". Tests: `tests/test_stage_a_produced_stager.py`, `tests/test_stage_a_produced_readahead.py` (every rule in both modes). |
| `PRISMAQUANT_STAGED_READ_STREAMS` | the staged mount's own `nconnect` (16 on `/stage/prewarm`); no split on a non-NFS mount or one publishing no `nconnect`/`rsize` | `residency_shard_reader.py` (`_read_shape`, `_read_span`) | How many concurrent reads one staged tensor's payload is split into. The reader served a span with one sequential `preadv` loop on one descriptor, so a staged byte crossed the 100 Gbps RDMA link on one stream however warm the tier was. The span is cut on the mount's `rsize` (1 MiB) and read on at most its `nconnect` threads into disjoint slices of the tensor's own buffer; both numbers are read from `/proc/self/mounts`, so there is no constant here to pick (principle 2). A span shorter than `nconnect x rsize` stays one read. Measured on sparklina against dl380g10's SSD stage, GLM-5.3-Flash shard 1, four declared tensors -- two 1.21 GiB and two 16 MiB -- server cache warm and the client page cache dropped before every read, 8 cold samples per size per arm (key `5cf5c36b85cc`). 1.21 GiB: 1 stream **1,575 MB/s**, 2 **3,435**, 4 **3,498**, 8 **3,536**, 16 **3,456**, 32 **3,431** -- 2.2x, flat from 2 on. 16 MiB: **1,417**, **2,217**, **2,406**, **2,562**, **2,483**, **1,355**, where the last is not a ceiling but the short-span guard: 16 MiB is shorter than 32 x 1 MiB, so that arm reads on one stream. `nconnect=16` is inside the large-span plateau and at the small span's best. CPU is not the ceiling (0.30 cores at 1 stream, 2.33 of 8 at 16) and `rchar` stays 1.002x the payload, so nothing is read twice. `1` restores the byte-identical single-stream read and is the before arm of that A/B. The pool is shared process-wide, so it cannot multiply against `PRISMAQUANT_LAYER_READ_THREADS` above. One failed chunk falls the whole tensor back to the pool, after every chunk has been waited for. Tests: `tests/test_residency_shard_reader.py`; tool: `tools/staged_read_stream_ab.py`. |
...
| `PRISMAQUANT_JOINT_PROGRESS_INTERVAL_S` | `60` | `joint_run_progress.py` (`interval_seconds`, read once per `compute_aura_cost_streamed`) | How often the joint AURA run emits one progress line and one PrismaBuild `progress-v1` commit: layers and partitions done, entries written, entries/s, and the residency resolver's `hits`/`misses`/`range_hits`/`range_misses`/`fallback_count` since the previous line. Time-based, so the cadence does not change with model size; the default is half of Rob's two-minute silence rule. A non-numeric or non-positive value is refused rather than replaced. With no `PRISMABUILD_ACTION_PROGRESS_PATH`/`_TOKEN` in the environment the line is still printed and nothing is committed, which is byte-identical to the pre-#741 behaviour. |
...
| `PRISMAQUANT_BF16_REDUCED_PRECISION_REDUCTION` | unset | `matmul_arithmetic.py` (`bf16_reduction_from_environment`), through `pin_matmul_arithmetic` at the Stage A, Stage B and joint AURA entry points and `tools/capture_glm_routing_replay.py`; validated by `tools/dispatch_joint_quanta.py` (`_container_wrap`) | The only value is `off`: it pins `torch.backends.cuda.matmul.allow_bf16_reduced_precision_reduction` to False and stamps `allow_bf16_reduced_precision_reduction: false` into `joint_aura.arithmetic_identity`, the Stage A chain arithmetic stamp and the Stage A run identity, so the join refuses a quantum run under another setting. Unset keeps PyTorch's default (True) and stamps nothing, byte-identical to the behavior before the flag was pinned. `on` spells the default and is refused, like any other value. Sealed in the campaign container spec's `env`, as `PRISMAQUANT_STAGE_B_REPLAY_REGIME` is. PQ #1028: at Stage B capture batch 8 the flag on moved the per-operator statistics 25 to 50 times further from an FP32-accumulation reference and changed 129 of 16384 routings; off matched batch 1. Tests: `tests/test_matmul_arithmetic.py`. |
| `PRISMAQUANT_STAGE_B_SPILL_ROOT` / `PRISMAQUANT_STAGE_B_SPILL_MAX_BYTES` | unset / unset | `joint_replay_spill.py` (`stage_b_spill_config`); forwarded by `tools/tessera_campaign_container.py` (`stage_b_spill_environment`); ceiling set by `tools/dispatch_joint_quanta.py` (`_container_wrap`) | Both set: the Stage B one-pass replay spills to an unlinked file under the local root (PQ #994). Declared in the campaign container spec's `env` beside a writable identity bind; PrismaBuild charges the ceiling, rounded up to whole GiB, to the box's `spool_gb` (PB #911). For a spill-sealed executable row the dispatcher replaces the spec's ceiling with the record's sealed `spill_bound.reservation_bytes`, and the quantum refuses any other ceiling; rows without an executable readset use the spec's value. Tests: `tests/test_stage_b_spill_ceiling_sealed.py`, `tests/test_stageb_one_pass_spill.py`. |
| `PRISMAQUANT_STAGE_A_KERNEL_PROFILE` / `PRISMAQUANT_STAGE_B_KERNEL_PROFILE` | unset / unset | `joint_cost_stage_a.py` (`KERNEL_PROFILE_ENV`); `joint_cost_quantum.py` (`_stage_b_kernel_profiler`) | `1` opens the `torch.profiler` kernel-time sessions: Stage A's around its capture (#899), Stage B's around the render-free chain and every retained window (PQ #1029). Each session's close sums every kernel it recorded, which took 15.6 s of one GLM-shaped proxy quantum's main thread (py-spy, PB `a3d982334572`). Unset, `counters.json` reports `kernel_active_s: None` with the reason in `kernel_profiler_error`, never a zero. Telemetry only: no identity, record or arithmetic changes. |
