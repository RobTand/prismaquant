...
| `image_manifest` | Exact Docker v2 or OCI platform manifest bytes whose digest is the pinned image reference -- or, when the pin is a multi-platform index/list, the exact index bytes whose digest is the pinned reference. Its `config.digest` explicitly relates Docker hosts reporting a config ID to hosts reporting the manifest ID. Original local IDs remain in each run. Unrelated IDs refuse. |
| `image_platform_manifest` | Present exactly when `image_manifest` is an index/list: the resolved platform manifest bytes. The loader requires their digest to be exactly one entry of the index, that entry's media type to be concrete and equal to the bytes' own, and the entry to name an architecture/os platform (recorded, never compared -- no run record states the executing host's architecture). The platform manifest's `config.digest` is the config identity, and a run's local ID may then be the index digest, the platform manifest digest, or the config digest. A platform manifest beside a concrete pinned manifest refuses, as does an index with none. |
