...
| `design/joint_aura_runtime_allocation.md` | Joint allocation over quality and runtime: `joint_aura_predicted_dloss` as a currency distinct from the scalar `output_mse_under_route_activation_contract`, the runtime price partition it requires, and why the two are never mixed in one table. | CURRENT design — NOT implemented: `runtime_provenance.admit_fixed_resources` refuses every full-engine v2 partition by design (prismaquant #420 open), so no runtime price exists to allocate against |
