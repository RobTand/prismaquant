"""Audit actual wider best-form batches, complete calls, energy and timed I/O."""
import collections
import csv
import gzip
import hashlib
...
for arm in result['call_profiles']:
    ...
    profiles.append(dict(label=arm['label'],trace=meta,step_count=len(steps),step_sum_us=sum(e['dur'] for e in steps),
        step_mean_us=statistics.mean(e['dur'] for e in steps),kernel_span_us=span,traceback_us=sum(e['dur'] for e in tb),
...
inputs = result['batch_call_inputs']
...
print(json.dumps(dict(status='PASS',action=key,receipt=receipt['receipt_sha256'],source=result['producer_source_sha256'],
    ...
    limitations=['A bounded endpoint comparison, not a completed pricing row or model.',
        ...
        'Old-source parity is evidence only; old receipts are not relabeled or adopted.',
        'The other host ran the other endpoint concurrently; both-host load is recorded.',
        'Both arms used 20 GiB explicit headroom and full 864-entry residency; production default unchanged.']),sort_keys=True))
